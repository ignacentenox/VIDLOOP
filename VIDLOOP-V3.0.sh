...
ZT_NETWORK_ID="${VIDLOOP_ZT_NETWORK_ID:-}"
...
log_info "Configurando ZeroTier network join..."
if [ -n "${ZT_NETWORK_ID:-}" ]; then
    log_info "Uniéndose a network ZeroTier: ${ZT_NETWORK_ID}..."
    
    sudo zerotier-cli join "${ZT_NETWORK_ID}"
    log_ok "ZeroTier: solicitud de join enviada a ${ZT_NETWORK_ID}"
    log_warn "Recordá autorizar este nodo en my.zerotier.com → network ${ZT_NETWORK_ID}"
    ZT_NODE_ID=$(sudo zerotier-cli info 2>/dev/null | awk '{print $3}' || echo 'desconocido')
    log_info "Node ID de este dispositivo: ${ZT_NODE_ID}"
else
    log_warn "VIDLOOP_ZT_NETWORK_ID no definido. ZeroTier instalado pero sin network asignada."
    log_warn "Ejecutá manualmente: sudo zerotier-cli join <network-id>"
fi
